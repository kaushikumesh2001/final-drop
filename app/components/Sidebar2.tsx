...
export default function Sidebar2({
  ...
  return (
    <div className="w-60 h-screen overflow-y-auto p-4 border-r-2 border-blue-700 flex flex-col gap-4">
     ...
     {/* Text Color */}
<Color
  onChange={(val: string) => onTextColor(val)}
/>
      ...
      <ImageUpload
        disabled={!activeStyle}
        onChange={(src: any) => onStyleChange("src", src)}
      />
     ...
     <Size
  value={activeStyle?.fontSize ?? 16}
  onChange={(val: number) => onFontSize(val)}
/>
      ...
      <Margin
        value={activeStyle?.margin ?? 0}
        onChange={(val: number) => onStyleChange("margin", val)}
      />

      {/* Padding */}
      <Padding
        value={activeStyle?.padding ?? 0}
        onChange={(val: number) => onStyleChange("padding", val)}
      />
    </div>
  );
}
